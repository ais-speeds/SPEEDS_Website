...
In order to install packages from GitHub, ensure Git is also set up on your machine[^scatter_plots-1]. If it is not installed, [download and install Git](https://git-scm.com/downloads). This will allow `devtools` to clone and install packages from GitHub directly.

[^scatter_plots-1]: The easiest way to check if Git is installed on your computer is by using the terminal (on macOS or Linux) or the command prompt (on Windows).
